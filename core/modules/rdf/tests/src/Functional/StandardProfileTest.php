...
class StandardProfileTest extends BrowserTestBase {
  ...
  protected function doFrontPageRdfaTests() {
    ...
    // Ensure that both article are listed.
    $this->assertEqual(2, count($graph->allOfType('http://schema.org/Article')), 'Two article found on front page.');
    ...
    // Test the properties that are common between pages and article and are
  ...
  protected function doArticleRdfaTests() {
    ...
    // Test the properties that are common between pages and article.
    $this->assertRdfaCommonNodeProperties($graph, $this->article, "Article");
    // Test properties that are displayed in both teaser and full mode.
    $this->assertRdfaArticleProperties($graph, "Article");
    // Test the comment properties displayed on article.
  ...
  /**
   * Tests output for properties held in common between article and pages.
   *
