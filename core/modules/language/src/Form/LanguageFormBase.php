  ...
  public function commonForm(array &$form) {
    ...
    if ($language->getId()) {
      ...
      $form['langcode'] = [
        ...
        '#default_value' => '',
        '#disabled' => FALSE,
        '#description' => $this->t('Use language codes as <a href=":w3ctags">defined by the W3C</a> for interoperability. <em>Examples: "en", "en-gb" and "zh-hant".</em>', [':w3ctags' => 'http://www.w3.org/International/article/language-tags/']),
      ];
    }
    $form['label'] = [
      '#type' => 'textfield',
  ...
  public function validateCommon(array $form, FormStateInterface $form_state) {
    ...
    if (!isset($form['langcode_view']) && !preg_match('@^[a-zA-Z]{1,8}(-[a-zA-Z0-9]{1,8})*$@', $form_state->getValue('langcode'))) {
      $form_state->setErrorByName('langcode', $this->t('%field must be a valid language tag as <a href=":url">defined by the W3C</a>.', [
        ...
        ':url' => 'http://www.w3.org/International/article/language-tags/',
      ]));
    }
