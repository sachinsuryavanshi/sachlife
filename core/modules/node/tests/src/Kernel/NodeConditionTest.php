...
class NodeConditionTest extends EntityKernelTestBase {
  ...
  public function testConditions() {
    ...
    $this->assertFalse($condition->execute(), 'Page type nodes fail node type checks for article.');
    ...
    $this->assertTrue($condition->execute(), 'Page type nodes pass node type checks for pages or article');
    // Check for the proper summary.
    ...
    $this->assertTrue($condition->execute(), 'Article type nodes pass node type checks for pages or article');

    // Set the context to the test node.
    $condition->setContextValue('node', $test);
    $this->assertFalse($condition->execute(), 'Test type nodes pass node type checks for pages or article');
